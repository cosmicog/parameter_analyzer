...
int main ( int argc, char** argv )
{
	...
	ros::param::get("parameter_analyzer/param_start", LASER_LIKELIHOOD_MAX_DIST);
	ros::param::get("parameter_analyzer/param_end", PARAM_END);
	ros::param::set("/amcl/laser_likelihood_max_dist", LASER_LIKELIHOOD_MAX_DIST);
...
{
	ROS_INFO("-------------------------------");
	ros::param::get("/amcl/laser_likelihood_max_dist", LASER_LIKELIHOOD_MAX_DIST);
	ROS_INFO("-- laser_likelihood_max_dist: %lf", LASER_LIKELIHOOD_MAX_DIST);
	ROS_INFO("-------------------------------\n");
...
void updateParams()
{
	LASER_LIKELIHOOD_MAX_DIST += INC;
	if (INC > 0)
	{
		if (LASER_LIKELIHOOD_MAX_DIST > PARAM_END)
		{
			ALL_DONE = true;
			return;
		}
...
{
	LASER_LIKELIHOOD_MAX_DIST = config.laser_lambda_short;
}
